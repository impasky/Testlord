...
async function oturum() {
  ...
  yaz(
    'Ordusuz oyuncuya hedef gösteriliyor',
    harita.oneri !== null,
  ...
  yaz(
    'Önerilen ordu mevcut kaynakla kurulabiliyor',
    eksik?.karsilanabilir === true,
  ...
  yaz(
    'Söylenen kadar asker eğitince hedef "alınabilir" oluyor',
    hedef?.kazanir === true,
  ...
  yaz(
    'Raporda şöhret arttı',
    (sonuc?.sonrasi.sohret ?? 0) > (sonuc?.oncesi.sohret ?? 0),
    `${sonuc?.oncesi.sohret} -> ${sonuc?.sonrasi.sohret}`,
  );
  yaz(
    'Raporda sıralama yükseldi',
    (sonuc?.sonrasi.sira ?? 99) <= (sonuc?.oncesi.sira ?? 0),
    `${sonuc?.oncesi.sira}. -> ${sonuc?.sonrasi.sira}.`,
  );
  yaz(
    'Raporda saatlik gelir arttı',
    say(sonuc?.sonrasi.gelir) > say(sonuc?.oncesi.gelir),
    `${say(sonuc?.oncesi.gelir)} -> ${say(sonuc?.sonrasi.gelir)}`,
  );
  yaz(
    'Raporda bölge sayısı arttı',
  ...
  if (benimBolge) {
    ...
    yaz(
      'Bölgenin geliştirme maliyeti var',
      detay.upgradeCost?.altin > 0,
    ...
    yaz(
      'Bölge bir aşama ilerledi',
      sonra.level === benimBolge.level + 1,
