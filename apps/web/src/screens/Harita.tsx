...
import { DunyaHaritasi } from '../components/harita/DunyaHaritasi';
...
/**
 * Bir öğenin yüksekliği, değiştikçe. Geri çağrı ref'i: panel çekmece ile
 * kart arasında değişince yeni öğe kendiliğinden gözleniyor.
 */
function useYukseklik(): [(el: HTMLElement | null) => void, number] {
  const [yukseklik, setYukseklik] = useState(0);
  const gozlemci = useRef<ResizeObserver | null>(null);
  const ref = useCallback((el: HTMLElement | null) => {
    gozlemci.current?.disconnect();
    if (!el) return;
    const olc = () => setYukseklik(el.offsetHeight);
    olc();
    gozlemci.current = new ResizeObserver(olc);
    gozlemci.current.observe(el);
  }, []);
  return [ref, yukseklik];
}
...
export function Harita({
  ...
  /** Diyar çekmecesi açık mı (olaylar, yürüyüşler, medeniyet). */
  const [cekmeceAcik, setCekmeceAcik] = useState(false);
  /** Bölge kartı tam yükseklikte mi; değilse yarım — harita görünür kalıyor. */
  const [sayfaTam, setSayfaTam] = useState(false);
  const tutamak = useRef<{ y: number; oynadi: boolean } | null>(null);
  // Alttaki panelin (kart ya da çekmece) GERÇEK yüksekliği: harita
  // seçimi bunun üstünde ortalıyor, gösterge bunun üstüne oturuyor.
  const [panelRef, altBosluk] = useYukseklik();
  ...
  const yoldaki = marches.data?.length ?? 0;

  return (
    <>
      {/*
        TAM EKRAN HARİTA (docs/23). Harita üst çubukla omurga şeridi
        arasındaki bütün alanı alıyor; eskiden ekranın ~%33'üydü ve
        altında diyar satırı, olaylar, yürüyüşler sırayla diziliydi.
        Onlar artık alttaki çekmecede. Bölge kartı haritanın ÜSTÜNDE
        yarım yükseklikte açılıyor: seçtiğin toprak ve komşuları
        görünür kalıyor.
      */}
      <div
        className="fixed inset-x-0 z-10 mx-auto max-w-lg overflow-hidden"
        style={{ top: 'var(--ust-bar)', bottom: 'calc(var(--alt-bar) + var(--omurga-serit))' }}
        data-harita-sayfasi=""
      >
        <h2 className="sr-only">{dunya.data?.ad ?? 'Dünya Haritası'}</h2>
        <DunyaHaritasi
          ...
          ittifakHedefi={
            harita.data.ittifakHedefi
              ? {
                  regionId: harita.data.ittifakHedefi.regionId,
                  etiket: harita.data.ittifakHedefi.etiket,
                }
              : null
          }
          benimMedeniyetId={lord.medeniyet?.id ?? null}
          altBosluk={altBosluk}
          gostergeGizli={seciliId !== null || cekmeceAcik}
          onSec={secimYap}
          onBosDokun={() => {
            if (seciliId !== null) kapat();
            else setCekmeceAcik(false);
          }}
        />

        {seciliId === null ? (
          /* ---- Diyar çekmecesi: haritanın altındaki her şey burada ---- */
          <div
            ref={panelRef}
            className="absolute inset-x-0 bottom-0 z-20 flex max-h-[72%] flex-col rounded-t-2xl border-t border-kenar bg-panel/95 backdrop-blur"
            data-diyar-cekmecesi=""
          >
            <button
              type="button"
            ...
            >
              ...
              {yoldaki > 0 && <Hap renk="var(--color-turuncu)">{`${yoldaki} ordu yolda`}</Hap>}
              <span
                aria-hidden
                className={`shrink-0 text-solgun transition-transform ${cekmeceAcik ? 'rotate-180' : ''}`}
              >
                ▴
              </span>
            </button>
              ...
              <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-3 pb-3">
                ...
                {harita.data.ittifakHedefi && (
                  <button
                    ...
                    onClick={() => secimYap(harita.data!.ittifakHedefi!.regionId)}
                  >
                    <span className="baslik shrink-0 text-[11px] text-mavi">{`[${harita.data.ittifakHedefi.etiket}] HEDEF`}</span>
                    <span className="min-w-0 flex-1 truncate text-[12px]">
                      {harita.data.regions.find(
                        (r) => r.id === harita.data!.ittifakHedefi!.regionId,
                      )?.name ?? 'Bölge'}
                    ...
                    <div className="space-y-2">
                      {marches.data?.map((m) => (
                        <Kart
                          ...
                          className="p-3"
                          vurgu={
                            m.kind === 'attack' ? 'var(--color-turuncu)' : 'var(--color-yesil)'
                          }
                        >
          ...
          /* ---- Bölge kartı: yarım yükseklik, yukarı çekince tam ---- */
          <div
            ref={panelRef}
            role="dialog"
            aria-label={bolge?.name ?? 'Bölge'}
            className="absolute inset-x-0 bottom-0 z-20 flex flex-col rounded-t-2xl border-t border-kenar bg-panel shadow-[0_-10px_30px_rgba(0,0,0,0.55)] transition-[height] duration-200 motion-reduce:transition-none"
            style={{ height: sayfaTam ? '92%' : '50%' }}
            data-bolge-sayfasi=""
          >
            <button
              type="button"
              aria-label={sayfaTam ? 'Kartı küçült' : 'Kartı büyüt'}
              className="flex h-7 w-full shrink-0 cursor-grab items-center justify-center"
              style={{ touchAction: 'none' }}
              onPointerDown={(e) => {
                tutamak.current = { y: e.clientY, oynadi: false };
              }}
              onPointerMove={(e) => {
                const t = tutamak.current;
                if (t && Math.abs(e.clientY - t.y) > 8) t.oynadi = true;
              }}
              onPointerUp={(e) => {
                const t = tutamak.current;
                if (!t?.oynadi) return;
                const d = e.clientY - t.y;
                if (d < -30) setSayfaTam(true);
                else if (d > 30) {
                  if (sayfaTam) setSayfaTam(false);
                  else kapat();
                }
              }}
              onClick={() => {
                // Sürükleme bittiyse tıklama ayrıca aç/kapa yapmasın.
                if (tutamak.current?.oynadi) {
                  tutamak.current = null;
                  return;
                }
                tutamak.current = null;
                setSayfaTam((t) => !t);
            ...
            >
              <span className="h-1 w-10 rounded-full bg-kenar" aria-hidden />
            </button>
            {bolge ? (
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-3">
                ...
                <div className="space-y-3 px-4 pt-3">
                  ...
                  */}
                        ...
                        (() => {
                          ...
                          return (
                            <Kart className="p-3" vurgu="var(--color-altin)">
                              ...
                              <KuyrukSeridi
                                kuyruklar={bolgeKuyrugu.filter(
                                  (q) => q.payload.regionId === bolge.id,
                                )}
                      ...
                      {bolge.store && (
                        <Kart className="p-3">
                          <h3 className="baslik mb-1 text-[11px] text-solgun">
                            Yağmalanabilir depo
                          </h3>
                      ...
                      {bolge.type !== 'taht' && (
                        <Kart className="border-kirmizi/30 p-3">
                          ...
                          <p className="text-[11px] text-solgun">
                            Bölge sahipsiz kalır, garnizondaki birlikler eve döner. Bakımı ağır
                            gelen ya da savunamadığın bir bölgeden böyle kurtulabilirsin.
                          </p>
                  ...
                  {bolge.garrisonVisible ? (
                    <Kart className="p-3">
                      ...
                      {Object.entries(bolge.garrison).filter(([, n]) => (n as number) > 0)
                        .length === 0 ? (
                  ...
                  {bolgeSavaslari.length > 0 && (
                    <Kart className="p-3">
                      <h3 className="baslik mb-2 text-[11px] text-solgun">
                        Bu Bölgedeki Savaşların
                      </h3>
                      <ul className="space-y-1.5">
